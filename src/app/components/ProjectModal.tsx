      ...
      >
        ...
        <div className="sticky top-0 z-10 flex flex-col bg-[#141414] rounded-t-2xl">
          <div className="flex items-center justify-between px-5 pt-4 pb-3 md:px-8 md:pt-6 md:pb-4">
            <span
              className="font-mono text-[12px] md:text-[14px] uppercase tracking-wide text-neutral-500"
            >
              {project.subtitle}
            </span>
            <button
              onClick={onClose}
              className="p-1.5 rounded-lg transition-colors text-neutral-500 hover:text-white hover:bg-white/5"
        ...
        <div ref={scrollRef} className="overflow-y-auto flex-1">
          ...
          <div className="px-5 pt-3.5 pb-3 md:px-8 md:pt-5 md:pb-4">
            <h2
              id="modal-title"
              className="text-[28px] md:text-[38px] lg:text-[42px] leading-[1.15] font-normal text-white"
            >
              {project.client}
            </h2>
          </div>
          ...
          <div className="relative mx-5 md:mx-8 overflow-hidden">
            ...
            <div
              ...
              style={{ backgroundColor: project.tintColor, opacity: 0.5 }}
            />
            {/* Dark gradient for stat legibility */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent pointer-events-none" />
            {/* Result stat overlay — bottom-right */}
            {project.result && (() => {
              const match = project.result.match(/^([\d%.+\-×xk]+)\s+(.+)$/i);
              const statValue = match ? match[1] : project.result;
              const statLabel = match ? match[2] : null;
              return (
                <div className="absolute bottom-4 right-4 md:bottom-6 md:right-6 text-right">
                  ...
                  {statLabel && (
                    <div className="font-mono text-[10px] md:text-[12px] uppercase tracking-wider text-white/70 mt-1">
                      {statLabel}
                    </div>
                  )}
                </div>
          ...
          <div className="px-5 pt-4 md:px-8 md:pt-5">
            <span
              className="font-mono text-[11px] md:text-[13px] uppercase tracking-wider block mb-3 text-neutral-500"
            >
              Overview (Epic)
            </span>
            <p className="leading-[1.7] text-neutral-300 text-[16px]">
          ...
          <div className="px-5 pt-4 pb-2 md:px-8 md:pt-5 md:pb-3">
            <span
              className="font-mono text-[11px] md:text-[13px] uppercase tracking-wider block mb-4 text-neutral-500"
            >
              Key Results
            </span>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
              {project.metrics.map((metric, index) => (
                ...
                >
                  <div
                    className="text-[22px] md:text-[26px] font-bold mb-1 text-white"
                  >
                    {metric.value}
                  </div>
                  <div
                    className="font-mono text-[10px] md:text-[11px] uppercase tracking-wider text-neutral-500"
                  >
                    {metric.label}
                  </div>
                </div>
              ...
              <div className="px-5 pt-4 md:px-8 md:pt-5">
                ...
                <div className="rounded-xl bg-white/[0.03] border border-white/[0.07] overflow-hidden">
                  ...
                  <div className="px-5 pt-5 pb-4 md:px-6">
                    <h3
                      className="text-[20px] md:text-[24px] leading-[1.25] text-white mb-3"
                      style={{ color: accent }}
                    >
                  ...
                  {!project.deepDive.image && project.deepDive.stat && (
                    <div className="px-5 pb-5 md:px-6">
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span className="font-mono text-[10px] md:text-[12px] uppercase tracking-wider text-neutral-500 mt-1">
                          {project.deepDive.stat.label}
                        </span>
                      </div>
                    </div>
